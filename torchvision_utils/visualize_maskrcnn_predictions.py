'''
I did not make this script, I got it from: https://gist.github.com/fmassa/d802c684512bcc99f54b714f10c96872

However, I made some minor changes in the visualization colors for my specific case, you can check out the original one on the link.
'''
import torch
import numpy as np
...
def overlay_boxes(image, predictions, colors):
    """
...
def overlay_mask(image, predictions, colors):
    """
...
def overlay_class_names(image, predictions):
    ...
    for box, score, label in zip(boxes, scores, labels):
        ...
        cv2.putText(
            image, s, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 10
        )
...
def predict(img, model):
    ...
    result = cv_img.copy()
    colors = compute_colors_for_labels(top_predictions['labels'])
    result = overlay_boxes(result, top_predictions, colors)
    if 'masks' in top_predictions:
        result = overlay_mask(result, top_predictions, colors)
